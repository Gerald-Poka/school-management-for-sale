...
document.getElementById('student_id').addEventListener('change', function() {
    ...
    document.getElementById('student_class').value = selectedOption.dataset.class;
    
    // Fetch fee structure for selected student
    fetch(`/admin/finance/invoices/fee-structure?student_id=${this.value}`)
        ...
        .then(feeStructures => {
            ...
            feeStructures.forEach((fee, index) => {
                const row = createFeeRow(index + 1, fee);
...
function createFeeRow(index, fee) {
    ...
    tr.innerHTML = `
        <td>${index}</td>
        <td>
            <input type="hidden" name="items[${index}][fee_structure_id]" value="${fee.id}">
            <input type="text" class="form-control bg-light border-0" value="${fee.fee_type.name}" readonly>
            <textarea class="form-control bg-light border-0" name="items[${index}][description]" rows="2">${fee.fee_type.description || ''}</textarea>
        </td>
        <td>
            <input type="number" class="form-control product-price bg-light border-0" 
                name="items[${index}][amount]" value="${fee.amount}" step="0.01">
        </td>
        <td colspan="2">
            <div class="text-end">
                <input type="text" class="form-control bg-light border-0 product-line-price" 
                    value="${fee.amount}" readonly>
            </div>
        </td>
    `;
...
document.getElementById('academic_level_id').addEventListener('change', function() {
    ...
    // Fetch regular fees
    fetch(`/admin/finance/invoices/fees/${academicLevelId}`)
        .then(response => response.json())
        .then(feeStructures => {
            const tbody = document.getElementById('newlink');
            tbody.innerHTML = ''; // Clear existing items
            ...
            feeStructures.forEach((fee, index) => {
                if (fee.fee_type.code.startsWith('TRANS')) return; // Skip transport fees
                const row = createFeeRow(index + 1, fee);
                tbody.appendChild(row);
            });
    ...
    fetch(`/admin/finance/invoices/transport-fees/${academicLevelId}`)
        .then(response => response.json())
        .then(transportFees => {
            const transportSelect = document.getElementById('transport_fee_id');
            ...
            transportFees.forEach(fee => {
                transportSelect.innerHTML += `
                    <option value="${fee.id}" data-amount="${fee.amount}">
                        ${fee.route_name} - ${fee.fee_type.name} (${fee.amount})
                    </option>
                `;
            });
        });
...
document.getElementById('transport_fee_id').addEventListener('change', function() {
    const option = this.options[this.selectedIndex];
    if (!option.value) return;

    const amount = option.dataset.amount;
    const routeName = option.text;

    // Add transport fee to invoice items
    const tbody = document.getElementById('newlink');
    const index = tbody.children.length + 1;
    
    const transportRow = createTransportFeeRow(index, {
        id: option.value,
        amount: amount,
...
function createTransportFeeRow(index, fee) {
    const tr = document.createElement('tr');
    tr.id = index;
    tr.className = 'product';
    ...
    tr.innerHTML = `
        ...
        <td>
            <input type="hidden" name="items[${index}][fee_structure_id]" value="${fee.id}">
            <input type="text" class="form-control bg-light border-0" value="Transport Fee" readonly>
            <textarea class="form-control bg-light border-0" name="items[${index}][description]" 
                rows="2" readonly>${fee.route_name}</textarea>
        </td>
        <td>
            <input type="number" class="form-control product-price bg-light border-0" 
                name="items[${index}][amount]" value="${fee.amount}" step="0.01" readonly>
        </td>
        <td colspan="2">
            <div class="text-end">
                <input type="text" class="form-control bg-light border-0 product-line-price" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    academicLevelSelect.addEventListener('change', function() {
        ...
        if (!levelId) {
            feeTableBody.innerHTML = '<tr><td colspan="5" class="text-center">Please select an academic level</td></tr>';
            totalAmountElement.textContent = '0.00';
            return;
        }

        // Fetch fee structures for selected academic level
        fetch(`/admin/finance/invoices/fee-structures/${levelId}`)
            .then(response => response.json())
            .then(feeStructures => {
                        ...
                        <td>
                            <div class="form-check">
                                <input type="checkbox" class="form-check-input fee-checkbox" 
                                    name="fee_structures[]" value="${fee.id}">
                            </div>
                        </td>
                        <td>${fee.fee_type.name}</td>
                        <td>${fee.fee_type.frequency}</td>
            ...
            .catch(error => {
                ...
                feeTableBody.innerHTML = '<tr><td colspan="5" class="text-center text-danger">Error loading fees</td></tr>';
            });
    });

    // Handle select all checkbox
    selectAllCheckbox.addEventListener('change', function() {
        const checkboxes = document.getElementsByClassName('fee-checkbox');
    ...
    feeTableBody.addEventListener('change', function(e) {
    ...
    form.addEventListener('submit', function(e) {
        ...
        const selectedFees = Array.from(document.getElementsByClassName('fee-checkbox'))
        ...
        const formData = {
            academic_level_id: academicLevelSelect.value,
            invoice_date: document.getElementById('invoice_date').value,
            due_date: document.getElementById('due_date').value,
            fee_structures: selectedFees
        };

        fetch('/admin/finance/invoices', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                window.location.href = `/admin/finance/invoices/${data.invoice_id}`;
            } else {
                alert(data.message);
            }
        })
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    academicLevelSelect.addEventListener('change', function() {
        ...
        // Fetch fee structures for the selected academic level
        fetch(`/system/Admin/finance/invoices/fee-structures/${levelId}`)
            .then(response => response.json())
            .then(feeStructures => {
                feeTableBody.innerHTML = '';
                ...
                if (feeStructures.length === 0) {
                    feeTableBody.innerHTML = '<tr><td colspan="5" class="text-center">No fee structures found for this academic level</td></tr>';
                }
